...
char* FastUInt32ToBufferLeft(std::uint32_t i, char* buffer, int base) {
  char* start = buffer;
  do {
    std::int32_t digit = i % base;
    char character;
    if (digit < 10) {
      character = '0' + digit;
    } else {
...
// Populates the provided buffer with an ASCII representation of the number.
char* FastInt32ToBufferLeft(std::int32_t i, char* buffer) {
  std::uint32_t u = i;
  if (i < 0) {
    *buffer++ = '-';
    u = -u;
...
char* StrCatInt32(char* main, int main_max_length, std::int32_t number) {
...
// Converts a number to a string and appends it to another.
char* StrCatUInt32(char* main, int main_max_length, std::uint32_t number, int base) {
  char number_string[kFastToBufferSize];
...
char* FastFloatToBufferLeft(float f, char* buffer) {
  ...
  const std::uint32_t sign_mask = 0x80000000;
  const std::uint32_t exponent_mask = 0x7f800000;
  const std::int32_t exponent_shift = 23;
  const std::int32_t exponent_bias = 127;
  const std::uint32_t fraction_mask = 0x007fffff;
  const std::uint32_t u = *reinterpret_cast<std::uint32_t*>(&f);
  const std::int32_t exponent =
      ((u & exponent_mask) >> exponent_shift) - exponent_bias;
  const std::uint32_t fraction = (u & fraction_mask);
  ...
  *current = 0;
  ...
  const std::int32_t scale_shifts_size = 13;
  const std::int8_t scale_shifts[13] = {3,  4,  8,  11, 13, 14, 17,
                                   18, 19, 20, 21, 22, 23};
	std::uint32_t scaled_fraction = fraction;
...
extern "C" void DebugLogInt32(std::int32_t i) {
  char number_string[kFastToBufferSize];
  FastInt32ToBufferLeft(i, number_string);
  DebugLog(number_string);
}

extern "C" void DebugLogUInt32(std::uint32_t i) {
  char number_string[kFastToBufferSize];
  FastUInt32ToBufferLeft(i, number_string, 10);
  DebugLog(number_string);
}

extern "C" void DebugLogHex(std::uint32_t i) {
  char number_string[kFastToBufferSize];
